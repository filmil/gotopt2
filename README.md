...
## `gotopt2-generator`: Reusable bash parsers

If you prefer to avoid the dependency on `gotopt2` at runtime or want to avoid dynamic code evaluation via `eval`, you can use the `gotopt2-generator` tool.

`gotopt2-generator` reads the identical YAML configuration as `gotopt2`, but instead of evaluating flags directly, it outputs a self-contained Bash function (`parse_args`) that you can embed directly into your scripts or source locally. The generated bash script implements exactly the same parsing logic and variable assignment behavior as `gotopt2`, including `--help` output and default value propagation.
...
This will generate `my_parser.sh` with the following content:
...
parse_args() {
  ...
  echo "# gotopt2:generated:begin"
    echo "gotopt2_baz='${gotopt2_baz}'"
    echo "gotopt2_foo='${gotopt2_foo}'"
  if [ ${#gotopt2_args__[@]} -gt 0 ]; then
    local args_vals=()
    local arg_name="gotopt2_args__"
    eval "for v in \"\${${arg_name}[@]}\"; do args_vals+=(\"'\$v'\"); done"
    local args_out="(${args_vals[*]:-})"
    echo "gotopt2_args__=${args_out}"
  fi
